...
polyFunctions_hda.sage stands for Hausdorff distance approximation.
...
def polyAsphericity(P, tol=1e-1, MaxIter = 1e2, verbose=0):
    ...
    for i in range(m):
        ...
        a.append(supp_fun_polyhedron(A.row(i), mP, showOutput=0)[0])

    B = matrix(RR, D.nrows(), D.ncols())
    b = []
    for j in range(l):
        [nDj, ymax] = supp_fun_polyhedron(D.row(j), Bn, showOutput=0)
...
def asphericity_iteration(xk, l, m, p, A, a, B, b, verbose = 0):
